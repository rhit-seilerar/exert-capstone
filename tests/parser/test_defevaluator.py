...
def test_defevaluator_none() -> None:
    ...
    assert de.evaluate_with_defs([tm.mk_int(0)]) == (False, False, DefMap(de.defs))
    assert de.evaluate_with_defs([tm.mk_int(1)]) == (True, True, DefMap(de.defs))
    assert de.evaluate_with_defs([tm.mk_id('abc')]) == (False, False, DefMap(de.defs))
    assert de.evaluate_with_defs([tm.mk_id('true')]) == (True, True, DefMap(de.defs))
...
def test_defevaluator_single() -> None:
    ...
    assert de.evaluate_with_defs([tm.mk_id('defined'), tm.mk_op('('),
        tm.mk_id('abc'), tm.mk_op(')')]) == \
        (True, True, DefMap(de.defs, initial = {'abc': Def(defined = True)}))
    de.defs['abc'] = Def(DefOption([]), defined = True)
    assert de.evaluate_with_defs([tm.mk_id('defined'), tm.mk_id('abc')]) == \
